...
function useResponsiveWheelSize() {
  const getSize = () => (typeof window === "undefined" ? 340 : window.innerWidth < 420 ? 240 : window.innerWidth < 760 ? 280 : 340);
...
export function RotationWheel({ members, currentTurn, amount, currency, size: sizeProp }) {
  const autoSize = useResponsiveWheelSize();
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, margin: "0 auto" }}>
      ...
      {members.map((m, i) => {
        ...
        const dotSize = isCurrent ? (isSmall ? 34 : 46) : (isSmall ? 28 : 38);
        return (
          <div key={m.id} style={{ position: "absolute", left: x, top: y, transform: "translate(-50%,-50%)" }}>
            <div style={{
              ...
              fontFamily: "Inter", fontWeight: 600, fontSize: isCurrent ? (isSmall ? 10 : 13) : (isSmall ? 9 : 11),
